...
class ClipbookUITests: XCTestCase {
  ...
  override func setUp() {
    ...
    if usesLegacyDoubleOptionLaunchArgument {
      app.launchArguments.append("enable-double-click-option-key")
    }
    app.launch()
  ...
  func testPopupWithDoubleOptionKey() {
    simulateOptionDoubleTap()
    waitUntilPoppedUp()
    assertExists(items[copy1])
    assertExists(items[copy2])
  ...
  func testPopupDoesNotOpenWithDoubleShift() {
    simulateShiftDoubleTap()
    assertNotExists(items[copy1])
  }

  func testPopupHotkeyDoesNotOpenInDoubleClickMode() {
    simulatePopupHotkey()
    assertNotExists(items[copy1])
  }

  func testPopupHotkeyDoesNotOpenWhileSettingsWindowIsOpen() {
    openPreferences()
  ...
  func testPopupWithDoubleOptionKeyDoesNotOpenWhileSettingsWindowIsOpen() {
    simulateOptionDoubleTap()
    waitUntilPoppedUp()
    app.staticTexts["Preferences…"].click()
    waitForPreferences()
    dismissPopupIfNeeded()
  ...
  private var usesLegacyDoubleOptionLaunchArgument: Bool {
    [
      "testPopupWithDoubleOptionKey",
      "testPopupDoesNotOpenWithDoubleShift",
      "testPopupHotkeyDoesNotOpenInDoubleClickMode",
      "testPopupWithDoubleOptionKeyDoesNotOpenWhileSettingsWindowIsOpen"
  ...
  private func waitForPreferences() {
    assertExists(app.staticTexts["Double-click to open"])
  }

  private func dismissPopupIfNeeded() {
    guard items[copy1].exists else { return }
  ...
  private func closePreferences() {
    ...
    assertNotExists(app.staticTexts["Double-click to open"])
  }
  ...
  private func simulateOptionDoubleTap() {
    simulateModifierDoubleTap(keyCode: CGKeyCode(kVK_Option), flags: .maskAlternate)
  }

  private func simulateShiftDoubleTap() {
    simulateModifierDoubleTap(keyCode: CGKeyCode(kVK_Shift), flags: .maskShift)
  }

  private func simulateModifierDoubleTap(keyCode: CGKeyCode, flags: CGEventFlags) {
